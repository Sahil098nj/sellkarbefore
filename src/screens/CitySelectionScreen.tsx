...
import { useCitiesQuery } from '../api/productsApi';
...
const CitySelectionScreen: React.FC = () => {
  ...
  const { variant, modelName } = route.params;
  ...
  const handleSelectCity = (cityName: string) => {
    navigation.navigate('DeviceQuestions', { variant, modelName, city: cityName });
  };
  ...
  return (
    <SafeAreaView style={styles.container}>
      <FlatList
        data={isLoading ? Array.from({ length: 8 }) : filteredCities}
        ...
        renderItem={({ item }: { item: { id: string; name: string; icon_url?: string | null } }) => {
          if (isLoading) {
            return (
